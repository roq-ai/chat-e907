...
import { getContactLists } from 'apiSdk/contact-lists';
import { ContactListInterface } from 'interfaces/contact-list';
import { getMessages } from 'apiSdk/messages';
import { MessageInterface } from 'interfaces/message';
import { getMediaFiles } from 'apiSdk/media-files';
import { MediaFilesInterface } from 'interfaces/media-files';

import { MediaFilesListPage as ShowCaseList } from 'pages/media-files';

function DashboardPage() {
  const { session } = useSession();

  const contactListStat = useSWR<PaginatedInterface<ContactListInterface>>(
    () => `/contact-lists`,
    () => getContactLists({ limit: 1, offset: 0 }),
  );
  ...
  const mediaFilesStat = useSWR<PaginatedInterface<MediaFilesInterface>>(
    () => `/media-files`,
    () => getMediaFiles({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Contact Lists',
      result: contactListStat?.data?.totalCount,
      isLoading: contactListStat.isLoading,
    ...
    {
      title: 'Media Files',
      result: mediaFilesStat?.data?.totalCount,
      isLoading: mediaFilesStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`media-files`} passHref legacyBehavior>
                  <Button
